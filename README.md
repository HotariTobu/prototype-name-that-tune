...
5. Each round:
   ...
   - **Wrong answer penalty (お手つき)**: if a player submits an incorrect answer, they are locked out from answering for a configurable duration (default: 5 seconds). Each player has a limited number of attempts per round (default: 3). Once all attempts are used, the player cannot answer for the rest of the round. The host can adjust the lockout duration, max attempts, or disable the penalty at room creation
